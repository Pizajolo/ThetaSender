...
contract MultiSend {
    ...
    uint public TFuelSendFee; // in basis points
    uint256 public TNT721SendFee; // in wei per token
    ...
    // Events
    event BulkSendTFuel(uint256 indexed numberOfAddresses, uint256 indexed totalAmount, bool fix, uint256 fee);
    event BulkSendTNT20(address indexed tokenAddress, uint256 indexed numberOfAddresses, uint256 indexed totalAmount,
        bool fix, uint256 fee);
    event BulkSendTNT721(uint256 indexed numberOfTokenAddresses, uint256 indexed numberOfAddresses, uint256 fee);
    event BulkSendTNT721Fixed(address indexed tokenAddress, uint256 indexed numberOfAddresses, uint256 fee);
    ...
    function bulkSendTFuel(address[] calldata addresses, uint256[] calldata amounts) public payable returns(bool success){
        ...
        uint256 fee = (TFuelSendFee * total * getFeeMultiplier(msg.sender)) /(100 * 10000);
    ...
    function bulkSendTFuelFixed(address[] calldata addresses, uint256 amount) public payable returns(bool success){
        ...
        emit BulkSendTFuel(addresses.length, requiredAmount, true, fee);
    ...
    function bulkSendTNT20(address tokenAddr, address[] calldata addresses, uint256[] calldata amounts) public returns(bool success){
        ...
        //ensure that the TFuel is enough to complete the transaction
        uint256 fee = (TNT20SendFee * total * getFeeMultiplier(msg.sender)) /(100 * 10000);
        uint256 requiredAmount = total + fee;
        ...
        ITNT20(tokenAddr).transferFrom(msg.sender, address(this), fee);

        emit BulkSendTNT20(tokenAddr, addresses.length, requiredAmount, false, fee);
    ...
    function bulkSendTNT20Fixed(address tokenAddr, address[] calldata addresses, uint256 amount) public returns(bool success){
        ...
        emit BulkSendTNT20(tokenAddr, addresses.length, requiredAmount, true, fee);
    ...
    function bulkSendTNT721(address[] calldata nftContracts, address[] calldata addresses, uint256[] calldata tokenIds) public payable returns(bool success){
        ...
        //ensure that the TFuel is enough to complete the transaction
        uint256 fee = addresses.length * TNT721SendFee * getFeeMultiplier(msg.sender)/100;
        ...
        emit BulkSendTNT721(nftContracts.length, addresses.length, fee);
    ...
    function bulkSendTNT721Fixed(address nftContract, address[] calldata addresses, uint256[] calldata tokenIds) public payable returns(bool success){
        ...
        emit BulkSendTNT721Fixed(nftContract, addresses.length, fee);

        return true;
    }

    function setTNT20SendFeeBasisPoints(uint feeBasisPoints) onlyAdmin external {
        require(feeBasisPoints <= 1000, "Sales Fee cant be higher than 10%");
        TNT20SendFee = feeBasisPoints;
    }

    function setTFuelSendFeeBasisPoints(uint feeBasisPoints) onlyAdmin external {
        require(feeBasisPoints <= 1000, "Sales Fee cant be higher than 10%");
        TFuelSendFee = feeBasisPoints;
    }
    ...
    function senderTNT20SendFee(uint256 amount) public view returns (uint256) {
        return (TNT20SendFee * amount * getFeeMultiplier(msg.sender)) /(100 * 10000);
    }

    function senderTFuelSendFee(uint256 amount) public view returns (uint256) {
        return (TFuelSendFee  * amount * getFeeMultiplier(msg.sender)) /(100 * 10000);
    }

    function senderTNT721SendFee(uint tokenAmount) public view returns (uint256) {
        return tokenAmount * TNT721SendFee * getFeeMultiplier(msg.sender)/100;
    }
